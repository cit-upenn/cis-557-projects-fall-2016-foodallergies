...
There are totally eight kinds of allergens set in seed file. Users should be able to set many allergens, while allergens should belong to many users. As a result, the relationship between user and allergens is set to: has_and_belongs_to_many. Then, each user is able to set their own allergens.
    User settings
A new RESTful member route is set as user/:id/settings. The corresponding actions are defined in users_controller. Update method is used to update the allergens belong to current user. Users are able to set their allergens through check boxes in the settings page. The setting view is also defined under the users view folder. 
    User Profile
